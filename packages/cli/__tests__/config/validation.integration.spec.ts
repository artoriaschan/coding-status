...
import { SettingsSchema, WidgetTypeEnum } from '@coding-status/widget-renderer';
...
describe('JSON Schema files', () => {
    ...
    it('should have settings.schema.json exist and parse as valid JSON', () => {
        const schemaPath = path.resolve(
            __dirname,
            '../../../widget-renderer/schemas/settings.schema.json'
        );
        expect(fs.existsSync(schemaPath)).toBe(true);
    ...
    it('should have config.schema.json contain $id, title, description metadata', () => {
        ...
        expect(schema.$id).toBe('https://coding-status.dev/schemas/config.schema.json');
        expect(schema.title).toBe('Coding Status Configuration');
        expect(schema.description).toBe(
            'Configuration for coding-status CLI - cloud provider usage statusline'
        );
    });
    ...
    it('should have settings.schema.json contain $id, title, description metadata', () => {
        ...
        expect(schema.$id).toBe('https://coding-status.dev/schemas/settings.schema.json');
        expect(schema.title).toBe('Coding Status Settings');
        expect(schema.description).toBe('Widget layout settings for coding-status statusline');
    });
    ...
    it('should have widget types enum in settings.schema.json', () => {
        ...
        const widgetDef =
            schema.definitions?.CDPSSettings?.properties?.rows?.items?.items?.properties?.widget;
    ...
    it('should show error type in formatted error', () => {
        ...
        if (!result.success) {
            ...
            const errorLine = lines.find(
                l => l.includes('Error:') && l.includes('invalid_enum_value')
            );
    ...
    it('should show fix suggestion for credentials field', () => {
        ...
        if (!result.success) {
            ...
            const suggestionLine = lines.find(
                l => l.includes('Suggestion:') && l.includes('credentials')
            );
