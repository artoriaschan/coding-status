...
describe('AdapterLoader', () => {
    ...
    describe('error handling', () => {
        ...
        it('AdapterInitError has packageName and reason', () => {
            const error = new AdapterInitError('@coding-status/adapter', 'Invalid credentials');
            expect(error.packageName).toBe('@coding-status/adapter');
            expect(error.message).toContain('Initialization failed');
            expect(error.message).toContain('Invalid credentials');
        });

        it('AdapterUsageError has dimension parameter', () => {
            const error = new AdapterUsageError('@coding-status/adapter', '5h', 'API timeout');
            expect(error.packageName).toBe('@coding-status/adapter');
