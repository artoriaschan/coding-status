 ...
 *
 * Per D-16:
 * - Creates ~/.coding-status/ directory
...
import { saveConfig, updateClaudeSettings } from '../../src/config/index.js';
...
describe('registerInit', () => {
    ...
    it('should have correct description for init command', () => {
        ...
        expect(initCommand?.description()).toBe(
            'Initialize ~/.coding-status/ directory and configure Claude Code statusline'
        );
    });
    ...
    it('should output message when Claude Code settings not found', async () => {
        ...
        // Assert
        expect(consoleLogSpy).toHaveBeenCalledWith('Claude Code settings.json not found (skipped)');
    });
});
