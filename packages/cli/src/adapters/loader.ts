...
import type { UsageAdapter } from '@coding-status/widget-renderer';

import { AdapterNotFoundError, AdapterLoadError, AdapterInitError } from './errors.js';
...
export class AdapterLoader {
     ...
     * 6. Cache instance
     *
     * @param packageName - npm package name (e.g., '@coding-status/usage-adapter-bailian')
    ...
    async getAdapter(
        ...
        try {
            ...
            if (!module.default) {
                throw new AdapterLoadError(packageName, 'Package has no default export');
            }
            ...
            if (error instanceof AdapterLoadError || error instanceof AdapterInitError) {
                throw error;
            }
