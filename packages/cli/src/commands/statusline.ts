 ...
 */
async function fetchAndRender(currentProvider: Provider, settings: Settings): Promise<string> {
    ...
    try {
        ...
        const results = await Promise.allSettled(dimensions.map(d => adapter.getUsage(d.key)));
