 ...
 *
 * Per D-16:
 * - Creates ~/.coding-status/ directory (via saveConfig mkdir)
...
import { saveSettings, DEFAULT_SETTINGS } from '@coding-status/widget-renderer';
...
export function registerInit(program: Command): void {
    program
        .command('init')
        .description('Initialize ~/.coding-status/ directory and configure Claude Code statusline')
        .action(async () => {
            console.log('Initializing coding-status...');
            ...
            console.log('Config directory: ~/.coding-status/');
            console.log('Next step: Run `coding-status add` to add a provider');
        });
}
