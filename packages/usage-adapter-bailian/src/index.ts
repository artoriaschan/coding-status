...
export const BailianAdapter: UsageAdapter = {
  ...
  async getUsage(dimension: string): Promise<number> {
    ...
    if (circuitState === 'reject') {
      if (cacheEntry && cacheEntry.data[dimension as keyof typeof cacheEntry.data] !== undefined) {
      ...
      throw new Error(`Circuit breaker open for ${config.providerName}, no cached data available`);
    }
