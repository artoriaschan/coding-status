...
describe('HttpClient', () => {
    ...
    describe('convenience methods', () => {
        ...
        it('post() sends POST request with body', async () => {
            mockFetch.mockResolvedValue(new Response(JSON.stringify({ created: true }), { status: 201 }));
    ...
    describe('error handling', () => {
        it('throws HttpError for 4xx response', async () => {
            mockFetch.mockResolvedValue(
                new Response(JSON.stringify({ code: 'NOT_FOUND' }), { status: 404, statusText: 'Not Found' })
            );
        ...
        it('throws HttpError for 5xx response (after retries exhausted)', async () => {
            mockFetch.mockResolvedValue(
                new Response(JSON.stringify({ code: 'SERVER_ERROR' }), { status: 500, statusText: 'Server Error' })
            );
    ...
    describe('network errors', () => {
        it('throws HttpNetworkError on DNS failure', async () => {
            ...
            const client = new HttpClient({ retry: 0 });
            await expect(client.get('https://nonexistent.example.com')).rejects.toThrow(HttpNetworkError);
        });
    });
