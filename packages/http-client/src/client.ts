...
export class HttpClient {
     ...
     */
    private async execute<T>(options: HttpRequestOptions, timeoutMs: number): Promise<HttpResponse<T>> {
        ...
        if (!response.ok) {
            ...
            throw new HttpError(response.status, bodyCode, response.statusText || `HTTP ${response.status}`);
        }
     ...
     */
    async get<T = unknown>(url: string, options?: Omit<HttpRequestOptions, 'url' | 'method' | 'body'>): Promise<HttpResponse<T>> {
     ...
     * @param options - Additional options (timeout, retry, auth, headers)
     */
    async post<T = unknown>(url: string, body?: unknown, options?: Omit<HttpRequestOptions, 'url' | 'method' | 'body'>): Promise<HttpResponse<T>> {
     ...
     * @param options - Additional options (timeout, retry, auth, headers)
     */
    async put<T = unknown>(url: string, body?: unknown, options?: Omit<HttpRequestOptions, 'url' | 'method' | 'body'>): Promise<HttpResponse<T>> {
     ...
     * @param options - Additional options (timeout, retry, auth, headers)
     */
    async delete<T = unknown>(url: string, options?: Omit<HttpRequestOptions, 'url' | 'method' | 'body'>): Promise<HttpResponse<T>> {
