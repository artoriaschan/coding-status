    ...
    describe('error type display', () => {
        it('should show invalid_type error code', () => {
            ...
            if (!result.success) {
                const lines = formatZodError(result.error, 'Config');
                expect(
                    lines.some(line => line.includes('invalid_type') || line.includes('Error'))
                ).toBe(true);
            }
        });
        ...
        it('should show required error code', () => {
            ...
            if (!result.success) {
                const lines = formatZodError(result.error, 'Config');
                expect(
                    lines.some(line => line.includes('required') || line.includes('Error'))
                ).toBe(true);
            }
        });
        ...
        it('should show too_small error code', () => {
            ...
            if (!result.success) {
                const lines = formatZodError(result.error, 'Config');
                expect(
                    lines.some(line => line.includes('too_small') || line.includes('Error'))
                ).toBe(true);
            }
        });
    });
    ...
    describe('fix suggestions', () => {
        it('should include suggestion for credentials field', () => {
            ...
            if (!result.success) {
                const lines = formatZodError(result.error, 'Config');
                expect(
                    lines.some(line => line.includes('Suggestion') || line.includes('credentials'))
                ).toBe(true);
            }
        });
...
describe('getFixSuggestion', () => {
    it('should return suggestion for credentials path', () => {
        ...
        expect(
            suggestion?.toLowerCase().includes('credentials') ||
