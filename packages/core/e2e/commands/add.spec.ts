...
describe('registerAdd', () => {
  ...
  it('should prompt for provider name with validation (min 1, max 32 chars)', async () => {
    ...
    const textCall = vi.mocked(p.text).mock.calls.find(
      (call) => call[0]?.message === 'Provider name',
    );
  ...
  it('should check for duplicate provider names in existing config', async () => {
    // Arrange
    vi.mocked(loadConfig).mockResolvedValue({
      providers: [{ name: 'existing-provider', type: 'bailian', packageName: '@cdps/usage-adapter-bailian', credentials: { accessKeyId: 'key', accessKeySecret: 'secret' } }],
  ...
  it('should not change current when adding second provider', async () => {
    ...
    vi.mocked(loadConfig).mockResolvedValue({
      providers: [{ name: 'existing', type: 'bailian', packageName: '@cdps/usage-adapter-bailian', credentials: { accessKeyId: 'key', accessKeySecret: 'secret' } }],
      current: 'existing',
      cacheTtl: 300,
    });
  ...
  it('should validate empty accessKeyId', async () => {
    ...
    const textCall = vi.mocked(p.text).mock.calls.find(
      (call) => call[0]?.message === 'AccessKey ID',
    );
    const validate = textCall?.[0]?.validate;
  ...
  it('should validate empty accessKeySecret', async () => {
    ...
    const passwordCall = vi.mocked(p.password).mock.calls.find(
      (call) => call[0]?.message === 'AccessKey Secret',
    );
